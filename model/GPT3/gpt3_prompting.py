...
# OpenAI GPT-3 Config
os.environ["OPENAI_API_KEY"] = "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"
...
for shot in shots:
    for split in splits:
        df = pd.read_csv('./data/eca-{}-cleaned.tsv'.format(split), delimiter="\t")
        exp_lst = []
        ...
        df.to_csv('./data/eca-{}-cleaned-exp-{}.tsv'.format(split, shot), sep='\t', index=False)
