...
def read_comet_file(filename):
    ...
    for i, line in enumerate(read_tsv):
        ...
        doc_id = line[4]
        xReact_output = line[5]
        
        data.append((clause, document, clause_label, emotion_label, doc_id, xReact_output))
...
def create_examples(lines, set_type):
        examples = []
        for i,(clause, document, clause_label, emotion_label, doc_id, xReact_response) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text_a = (clause, document, doc_id, xReact_response)
            text_b = None
...
class CometEvalDataset(TensorDataset):
    def __init__(self, args, eval_file_type, tokenizers):
        self.args = args
        self.eval_file_type = eval_file_type
        self.label_tuple = get_labels()
        self.tokenizers = tokenizers
        # self.eval_data = read_comet_file(os.path.join(args.data_dir, "comet-dev-pair.tsv")) if eval_file_type == 'dev' else read_comet_file(os.path.join(args.data_dir, "comet-test-pair.tsv"))  # a list of tuples
        self.eval_data = read_comet_file(os.path.join(args.data_dir, "comet-train-pair-sample-xReact.tsv"))
        self.idx_to_doc_id = {}
        idx = -1  # batch index
        doc_id_prev = -1
    ...
    def __getitem__(self, idx):
        ...
        for data in self.eval_data:
            if data[4] == doc_id:
                ...
                break
        examples = create_examples(lines, self.eval_file_type)
...
class COMETProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_comet_tsv(os.path.join(data_dir, "comet-train-pair-sample-xReact.tsv")), "train")
    ...
    def _create_examples(self,lines,set_type):
        examples = []
        for i, (clause, document, clause_label, emotion_label, doc_id, xReact_output) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text_a = (clause, document, doc_id, xReact_output)
            text_b = None
            labels = (clause_label, emotion_label)
            examples.append(InputExample(guid=guid,text_a=text_a,text_b=text_b,label=labels))
        return examples
...
def convert_examples_to_emotion_features(args, examples, label_tuple, max_seq_length, tokenizers):
    ...
    # token_label_map = {label : i for i, label in enumerate(token_labels, 1)}
    emotion_label_map = {label : i for i, label in enumerate(emotion_labels, 0)}
...
def convert_comet_examples_to_features_v1(args, examples, label_tuple, max_seq_length, tokenizers):
    """Loads a data file into a list of `InputBatch`s."""
    token_labels, emotion_labels = label_tuple
    ...
    features = []
    for (ex_index, example) in enumerate(examples):
        clause = example.text_a[0].strip()
        document = re.sub('\s+',' ', example.text_a[1]).strip().split(' ')
        # doc_id = example.text_a[2]
        token_label, emotion_label = example.label
        bert_tokens = []
        valid = []  # TODO: what is this?
        label_mask = []
        ...
        # label_mask.insert(0,1)
        # label_ids.append(token_label_map["[CLS]"])
        for _, token in enumerate(bert_tokens):
            bert_ntokens.append(token)
            segment_ids.append(0)
            # if len(labels) > i:
            #     label_ids.append(token_label_map[labels[i]])
        ...
        for label in token_label.split(' '):
            label_ids.append(token_label_map[label])

        clause_bert_ids = bert_tokenizer.convert_tokens_to_ids(bert_tokenizer.tokenize(clause.lower()))

        # preprocessing for comet
        # embed()
        query = "{} {} [GEN]".format(clause.lower(), 'xReact')
        # embed()
        # query = "{} {} [GEN]".format(clause.lower(), 'xEffect')
        tokenized_sequence = comet_tokenizer.tokenize(query)
        # comet_input_ids = comet_tokenizer.encode(tokenized_sequence, truncation=True, max_length=args.max_comet_seq_length, padding="max_length")
        comet_input_ids = comet_tokenizer.encode(tokenized_sequence, truncation=True)
        ...
        while len(comet_input_ids) < args.max_comet_seq_length:  # padding for comet
            comet_input_ids.append(1)
            label_ids.append(0)
            label_mask.append(0)

        while len(clause_bert_ids) < args.max_comet_seq_length:  # padding for clause
            clause_bert_ids.append(0)
        ...
        assert len(input_mask) == max_seq_length
        assert len(segment_ids) == max_seq_length
        assert len(label_ids) == args.max_comet_seq_length
        assert len(valid) == max_seq_length
        assert len(label_mask) == args.max_comet_seq_length
        ...
        # embed()
        all_input_ids = bert_input_ids + comet_input_ids + clause_bert_ids  # 1 x (bert_max_seq_len + comet_max_len + 1)

        # if 'comet' in args.model_class:
        assert len(all_input_ids) == (args.max_seq_length + args.max_comet_seq_length + len(clause_bert_ids))
...
#         # preprocessing for comet
#         query = "{} {} [GEN]".format(' '.join(clause).lower(), 'xReact')
#         # query = "{} {} [GEN]".format(clause.lower(), 'xEffect')
#         tokenized_sequence = comet_tokenizer.tokenize(query)
#         # comet_input_ids = comet_tokenizer.encode(tokenized_sequence, truncation=True, max_length=args.max_comet_seq_length, padding="max_length")
#         comet_input_ids = comet_tokenizer.encode(tokenized_sequence, truncation=True)
...
#         # if 'comet' in args.model_class:
#         assert len(all_input_ids) == (args.max_seq_length + args.max_comet_seq_length)
...
##### BERT-Clause + xReact joint input to Bert #####
def convert_comet_examples_to_features(args, examples, label_tuple, max_seq_length, tokenizers):
    """Loads a data file into a list of `InputBatch`s."""
    token_labels, emotion_labels = label_tuple
